...
import pandas as pd
import pickle
import data_norm_cluster_input as data_norm
...
def sub_fragments_extract(InputDim=(128,128),OutputDim=(100,100),Stride=(50,50),image_type='all',train=True,reflection=False):
    ...
    if train:
        df_all = pickle.load(open("/home/mao47/Kaggle_nuclei/inputs/train_df.p","rb"))
    else:
        df_all = pickle.load(open("/home/mao47/Kaggle_nuclei/inputs/test_df.p","rb"))
    if image_type == 'all':
        df = df_all
    ...
    for index,row in df.iterrows():
        ...
        img = row['Image']
        ImageId = row['ImageId']
        cluster = df.loc[df['ImageId']==ImageId,'hsv_cluster'].item()
        hsv_input=np.reshape(df.loc[df['ImageId']==ImageId,'hsv_dominant'].item(), (1,3))
        hsv_layer=np.ones((np.shape(img)[0],np.shape(img)[1],1))*hsv_input
        rgb_background=hsv2rgb(hsv_layer)
        img=np.concatenate((img,rgb_background),axis=2)
...
def sub_fragments_extract_extra(InputDim=(128,128),OutputDim=(100,100),Stride=(50,50),reflection=False):
    print('Loading pickle file ...')
    df = pickle.load(open("/home/mao47/Kaggle_nuclei/inputs/train_df.p","rb"))
