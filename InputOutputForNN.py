...
def sub_fragments_extract(InputDim=(128,128),OutputDim=(68,68),Stride=(34,34),image_type='all',train=True,reflection=True):
    print('Loading pickle file ...')
    if train:
        df_all = getInput() #pickle.load(open("../inputs/train_df.p","rb"))
    else:
    ...
    for index,row in df.iterrows():
        ...
        img = row['ImageInput']
        ImageId = row['ImageId']
        ImageShape = row['ImageInput'].shape[:2]
