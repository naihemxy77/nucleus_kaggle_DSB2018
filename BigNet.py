...
def Shrink_Block(X,f,x,y):
    X0 = Conv2D(filters=f//2,kernel_size=(2,2),kernel_initializer='he_normal', padding='valid')(X)
    ...
    X3 = Conv2D(filters=f//2,kernel_size=(x+y-1,x+y-1),kernel_initializer='he_normal', padding='valid')(X)
...
def model_gen(input_shape = [192,192,3],f=20,nClass=1,init_lr=0.02):
    ...
    X1 = Shrink_Block(X=conv1,f=f,x=6,y=3)
    X1_shortcut = X1
    
    X2 = Shrink_Block(X=X1,f=f,x=9,y=4)
    X2_shortcut = X2
    
    X3 = Shrink_Block(X=X2,f=f,x=12,y=5)
    X3 = Dropout(0.25)(X3)
    X4 = Shrink_Block(X=X3,f=f,x=12,y=5)
    
    X5 = MergeCrop([X4,X2_shortcut])
    X5 = Shrink_Block(X=X5,f=f,x=15,y=4)
    
    X6 = MergeCrop([X5,X1_shortcut])
    X6 = Shrink_Block(X=X6,f=f,x=20,y=3)
